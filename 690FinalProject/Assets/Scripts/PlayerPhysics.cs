...
public class PlayerPhysics : NetworkBehaviour
{
    ...
    public void ApplyForce(Vector3 force, ForceMode2D FMode)//apply force on the client-side to reduce the appearance of lag and then apply it on the server-side
    {
    ...
    [Command]//function that runs on server when called by a client
    public void CmdApplyForce(Vector3 force, ForceMode2D FMode)
    {
